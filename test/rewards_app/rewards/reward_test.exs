defmodule RewardsApp.Rewards.RewardTest do
  ...
  test "changeset/2 validates points value" do
    ...
    changeset = Reward.changeset(%Reward{}, %{points: 0, receiver_id: receiver.id, pool_id: pool_id})
    assert {"must be greater than %{number}", _} = changeset.errors[:points]

    changeset = Reward.changeset(%Reward{}, %{points: 1, receiver_id: receiver.id, pool_id: pool_id})
    refute changeset.errors[:points]

    changeset = Reward.changeset(%Reward{}, %{points: 2, receiver_id: receiver.id, pool_id: pool_id})
    refute changeset.errors[:points]

    changeset = Reward.changeset(%Reward{}, %{points: 3, receiver_id: receiver.id, pool_id: pool_id})
